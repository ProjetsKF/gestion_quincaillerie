<?php

/* Vérification admin */

require_once '../auth_admin.php';

/* Connexion base de données */

require_once '../bd/database.php';

/* Charger les utilisateurs */

$sql = "SELECT * FROM utilisateur ORDER BY idutil DESC";
$stmt = $pdo->prepare($sql);
$stmt->execute();
$users = $stmt->fetchAll();

?>

<!DOCTYPE html>
<html lang="en">

<head>

    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="">
    <meta name="author" content="">

    <title>BISIKOMASH - Utilisateurs</title>

    <!-- Custom fonts for this template-->
    <link href="../vendor/fontawesome-free/css/all.min.css" rel="stylesheet" type="text/css">
    <link
        href="https://fonts.googleapis.com/css?family=Nunito:200,200i,300,300i,400,400i,600,600i,700,700i,800,800i,900,900i"
        rel="stylesheet">

    <!-- Custom styles for this template-->
    <link href="../css/sb-admin-2.min.css" rel="stylesheet">

</head>

<body id="page-top">

    <!-- Page Wrapper -->
    <div id="wrapper">

         <!-- MENU -->

             <?php include("../menu.php"); ?>

        <!-- FIN MENU -->

        <!-- Content Wrapper -->
        <div id="content-wrapper" class="d-flex flex-column">

            <!-- Main Content -->
            <div id="content">

               
            <!-- Topbar -->

                 <?php include("../topbar.php"); ?>

            <!-- End of Topbar -->

                <!-- Begin Page Content -->
                 <h1 class="h3 mb-4 text-gray-800">Gestion des utilisateurs</h1>
                <div class="container-fluid">

                    <!-- Page Heading -->
                    <table class="table table-hover">

                <thead>
                <tr>
                    <th>Rôles</th>
                    <th>ID</th>
                    <th>Nom</th>
                    <th>Email</th>
                    <th>Statut</th>
                    <th>Actions</th>
                   
                </tr>
                </thead>

              <tbody>

                            <?php foreach($users as $u): ?>

                            <tr>
                            <td>

                                    <form method="POST" action="changer_role.php">

                                    <input type="hidden" name="id" value="<?php echo $u['idutil']; ?>">

                                    <select name="role" class="form-control form-control-sm">

                                    <option value="0" <?php if($u['rol']==0) echo "selected"; ?>>
                                    Utilisateur
                                    </option>

                                    <option value="1" <?php if($u['rol']==1) echo "selected"; ?>>
                                    Administrateur
                                    </option>

                                    </select>

                                    <button class="btn btn-primary btn-sm mt-1">
                                    Modifier
                                    </button>

                                    </form>

                            </td>

                            <td><?php echo $u['idutil']; ?></td>

                            <td>
                            <?php echo htmlspecialchars($u['prenom'].' '.$u['nom']); ?>
                            </td>

                            <td>
                            <?php echo htmlspecialchars($u['email']); ?>
                            </td>

                            <td>

                            <?php if($u['statut']=="En attente"): ?>

                            <span class="badge badge-warning">
                            En attente
                            </span>

                            <?php elseif($u['statut']=="Actif"): ?>

                            <span class="badge badge-success">
                            Actif
                            </span>

                            <?php else: ?>

                            <span class="badge badge-danger">
                            Suspendu
                            </span>

                            <?php endif; ?>

                            </td>

                            <td>

                            <?php if($u['statut']=="En attente"): ?>

                            <a href="activer.php?id=<?php echo $u['idutil']; ?>"
                            class="btn btn-success btn-sm">
                            Activer
                            </a>

                            <?php elseif($u['statut']=="Actif"): ?>

                            <a href="suspendre.php?id=<?php echo $u['idutil']; ?>"
                            class="btn btn-warning btn-sm"
                            onclick="return confirm('Suspendre cet utilisateur ?');">
                            Suspendre
                            </a>

                            <?php else: ?>

                            <a href="activer.php?id=<?php echo $u['idutil']; ?>"
                            class="btn btn-primary btn-sm">
                            Réactiver
                            </a>

                            <?php endif; ?>

                            </td>

                            </tr>

                            <?php endforeach; ?>

                            </tbody>

                 </table>

                </div>
                <!-- /.container-fluid -->

            </div>
            <!-- End of Main Content -->

             <!-- PIED DE PAGE -->

                     <?php include("../pieds.php"); ?>

             <!-- FIN PIED DE PAGE -->


        </div>
        <!-- End of Content Wrapper -->

    </div>
    <!-- End of Page Wrapper -->

    <!-- Scroll to Top Button-->
    <a class="scroll-to-top rounded" href="#page-top">
        <i class="fas fa-angle-up"></i>
    </a>

    <!-- Logout Modal-->
    <div class="modal fade" id="logoutModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Ready to Leave?</h5>
                    <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <div class="modal-body">Select "Logout" below if you are ready to end your current session.</div>
                <div class="modal-footer">
                    <button class="btn btn-secondary" type="button" data-dismiss="modal">Cancel</button>
                    <a class="btn btn-primary" href="login.html">Logout</a>
                </div>
            </div>
        </div>
    </div>

<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
<script src="../vendor/jquery/jquery.min.js"></script>
<script src="../vendor/bootstrap/js/bootstrap.bundle.min.js"></script>
<script src="../vendor/jquery-easing/jquery.easing.min.js"></script>
<script src="../js/sb-admin-2.min.js"></script>

</body>

</html>